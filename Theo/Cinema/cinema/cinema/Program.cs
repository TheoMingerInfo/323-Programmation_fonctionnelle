...
class Program
{
    static void Main(string[] args)
    {
        ...
        Func<Movie, bool> noFun = movie => movie.Genre != "Comédie" && movie.Genre != "Drame";

        Func<Movie, bool> Ratings = movie => movie.Rating <= 7;

        Func<Movie, bool> Date = movie => movie.Year < 2000;

        Func<Movie, bool> Doublage = movie => movie.LanguageOptions.Contains("Français");
...
public class Movie
{
    ...
    public string Genre { get; set; }              // Genre (ex: Comédie, Drame)
    public double Rating { get; set; }             // Note/évaluation (ex: 8.3)
    public int Year { get; set; }                   // Année de sortie
    public string[] LanguageOptions { get; set; }  // Langues disponibles
    public string[] StreamingPlatforms { get; set; } // Plateformes de streaming
}
